...
- 低噪声功能基线：只表示已知稳定、低噪声的 85 功能对照；不证明字节级官服原版。
- 动作化基座：只对完整 PVF SHA 绑定的研发事实负责。归档 PVF 与当前客户端资源若不是同一快照，必须标 `resourceTemporalAlignment: unknown`。
- 内容兼容上界：只作内容量和兼容压力上界；其独有内容不能称为官方字段或官方语义。
