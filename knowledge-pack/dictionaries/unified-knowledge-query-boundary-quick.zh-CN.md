...
每次返回 `kind`、portable 或 task-supplied artifact 标识与完整 SHA256、query、match/return/truncated 摘要、`results` 数组、证据/写入边界和必要的底层 delegated 元数据。
...
- planner 不是导入计划，unresolved 不得静默忽略；client `custom-only` 不代表官方，资源 present 不代表实机正确。
