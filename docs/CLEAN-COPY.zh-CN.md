...
5. 直接使用 `workbench.bat pvf-read`、`workbench.bat pvf-index`、`workbench.bat pvf-change`；随包后端不需要宿主 MCP、已下架插件或额外知识目录。若 `check` 显示只读降级，可继续读取和 dry-run，但必须修复 native 后才能 apply。
